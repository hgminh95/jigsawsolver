...
void GeneticSolver::Solve()
{
	...
	for (int i = 0; i < MAX_GENERATION_NUMBER; i++){
...
void GeneticSolver::removeSameChromosome()
{
	...
	for (int i = 1; i < crrPopulation.size(); i++)
	{
...
void GeneticSolver::addNewPiece(const Segment& father, const Segment& mother, Segment& child, int part, const Position& pos){
	...
	// delete from not selected piece
	for (int i = 0; i <= notSelectedPiece.size(); i++)
	...
	for (int i = 0; i < 4; i++){
		Position newPos;
		newPos.X = pos.X + PROCON::hx[i];
		newPos.Y = pos.Y + PROCON::hy[i];

		if (!child.isEmpty(newPos)) continue;

		// if both father and mother agree
		...
		// if only father (mother) agree and he is best buddy
...
Segment GeneticSolver::crossover(Segment& father, Segment& mother){
	...
	for (int j = 0; j < nColumns; j++) notSelectedPiece.push_back(i * nColumns + j);
	...
	while (child.count() < nRows * nColumns){
		if (vector_1.size() > 0)
		{
			...
			else addPieceInThirdList(father, mother, child);
		}
...
void GeneticSolver::addPieceInThirdList(const Segment& father, const Segment& mother, Segment& child)
{
	...
	for (int i = 1; i < notSelectedPiece.size(); i++)
	{
